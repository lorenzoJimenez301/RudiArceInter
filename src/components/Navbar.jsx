...
//                 className='bg-green-500 h-screen w-full absolute flex-col font-poppins font-light flex gap-8 justify-center items-center text-xl lg:hidden z-50'>
//                 <ul className='gap-10 flex justify-center items-center flex-col w-full bg-red-600'>
//                     <motion.li initial={{ opacity: 0, y: 50 }} animate={{ opacity: isOpen ? 1 : 0, y: isOpen ? 0 : 50 }} transition={{ duration: 0.3, stiffness: 1000, velocity: -100 }}><Link onClick={toggleMenu} href='/'>Inicio</Link></motion.li>
//                     <motion.li initial={{ opacity: 0, y: 50 }} animate={{ opacity: isOpen ? 1 : 0, y: isOpen ? 0 : 50 }} transition={{ duration: 0.4, stiffness: 1000, velocity: -100 }}><Link onClick={toggleMenu} href='/seguros'>Seguro de Vida</Link></motion.li>
//                     <motion.li initial={{ opacity: 0, y: 50 }} animate={{ opacity: isOpen ? 1 : 0, y: isOpen ? 0 : 50 }} transition={{ duration: 0.5, stiffness: 1000, velocity: -100 }}><Link href='/sobreMi'>Sobre Mi</Link></motion.li>
//                     <motion.li initial={{ opacity: 0, y: 50 }} animate={{ opacity: isOpen ? 1 : 0, y: isOpen ? 0 : 50 }} transition={{ duration: 0.6, stiffness: 1000, velocity: -100 }}><Link href='/constacto'>Contacto</Link></motion.li>
//                     <motion.li initial={{ opacity: 0, y: 50 }} animate={{ opacity: isOpen ? 1 : 0, y: isOpen ? 0 : 50 }} transition={{ duration: 0.6, stiffness: 1000, velocity: -100 }}><Link href='https://wa.me/+50683105134?text=Hola%21%2C%20Brindame%20m%C3%A1s%20informaci%C3%B3n%20sobre%20los%20seguros%2C%20por%20favor.' target='_blank' className='bg-azulCustom py-[11px] px-[30px] font-medium flex items-center justify-center gap-3'>Whatsapp<BsWhatsapp /></Link></motion.li>
//                 </ul>
//                 <div>
...
const Navbar = () => {
    ...
    return (
        <nav className='text-center w-full text-white text-xs md:text-lg'>
            <div className={`flex w-full items-center justify-between bg-blackCustom z-20 ${isOpen ? 'fixed' : ''} lg:relative top-0 left-0 right-0`}>
                <Image priority className='ml-2 lg:ml-5' src={navLogo} alt='Logo' height={80} />
                {isOpen ?
                    <TfiClose onClick={toggleMenu} className='text-white text-[33px] mr-5 cursor-pointer block lg:hidden' /> :
                    <CiMenuFries onClick={toggleMenu} className='text-white text-[33px] mr-5 cursor-pointer block lg:hidden' />
